  ...
  return <div className="relative min-h-screen">
      ...
      <div className="relative z-10 pt-12 pb-12">
        <div className="w-full px-4">
          ...
          <div role="list" aria-label="Career positions" className="space-y-3 m-10 ">
            {positions.map((position, index) => <Link key={position.id} to={`/position/${position.id}`} className="block group" onMouseEnter={() => handleMouseEnter(position.id)} onMouseLeave={handleMouseLeave} onFocus={() => handleMouseEnter(position.id)} onBlur={handleMouseLeave} role="listitem" aria-label={`View details for ${position.title} at ${position.company}`}>
                <div className="backdrop-blur-lg border-0 rounded-lg hover-lift hover:border-primary/50 hover:border-2 transition-all duration-300 py-0 bg-[#1b1f1b]/[0.24] px-3 ">
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      <div className="flex flex-col md:flex-row  md:justify-between mb-0">
                        <div>
                          <h2 className="text-xl md:text-2xl text-primary group-hover:text-foreground transition-colors font-normal">
                            {position.title}
                          </h2>
                          <p className="text-lg text-accent group-hover:text-foreground font-normal">
                            {position.company}
                          </p>
                        </div>
                        <div className="flex flex-shrink flex-row mt-1 md:mt-0 justify-end group-hover:text-foreground text-right text-primary-foreground ">
